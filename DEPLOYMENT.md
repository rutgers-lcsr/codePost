...
```bash
git clone <repo-url>/codePost.git
git clone <repo-url>/codePost-api.git
git clone <repo-url>/codePost-ui.git
```
...
make setup
# Then edit .env — change all CHANGE_ME values

# Option B: Use the interactive script
python ../codePost-api/scripts/create_env.py --output .env
```

At minimum, you must set:

- `SECRET_KEY` — Django secret (auto-generated by the script)
- `FIELD_ENCRYPTION_KEY` — Database field encryption key (auto-generated)
...
- `WORKER_SHELL_SHARED_SECRET` — Worker relay secret (auto-generated)
...
4. Start with auto-renewal:
    ```bash
    docker compose -f docker-compose.production.yml --profile certbot up -d
    ```
...
```bash
docker compose -f docker-compose.production.yml logs api | grep "API Token"
```
...
| Assignment datasets | `codepost-datasets` volume | Volume backup or rsync |
...
| UI (React)    | 443           | —            | Internal only, proxied           |

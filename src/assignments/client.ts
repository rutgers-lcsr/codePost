...
import {
    ...
    AssignmentSubmissionTestsResponse,
    AssignmentTestsResponse,
    QueryAssignmentList,
    RubricResponse,
} from "./types";

const basicAssignmentFunctions = BasicFunctions<Assignment>(
    "/assignments",
    AssignmentSchema,
);

export const Assignments = {
    ...basicAssignmentFunctions,
    getAssignment: async (assignmentId: number) => {
        return await CodePostHTTP.get<Assignment>(
            `/assignments/${assignmentId}/`,
        );
    },
    getComments: async (assignmentId: number) => {
        return await CodePostHTTP.get<CommentSerializer>(
            `/assignments/${assignmentId}/comments/`,
        );
    },
    getQueueLength: async (assignmentId: number) => {
        return await CodePostHTTP.get<AssignmentQueueLength>(
            `/assignments/${assignmentId}/queueLength/`,
        );
    },
    getRubric: async (assignmentId: number) => {
        return await CodePostHTTP.get<RubricResponse>(
    ...
    getNextUnassigned: async (assignmentId: number, section?: string) => {
        ...
        return await CodePostHTTP.get<Submission | AnonymousSubmission>(
            `/assignments/${assignmentId}/drawUnassigned/`,
            params,
        );
    },
    ...
    getSubmissionHistories: async (
        assignmentId: number,
        query?: AssignmentSubmissionHistoryQuery,
    ) => {
        const params = getQueryParams(query);
        return await CodePostHTTP.get<SubmissionHistory[]>(
            `/assignments/${assignmentId}/submissionHistories/`,
        );
    },
    getTests: async (assignmentId: number) => {
        return await CodePostHTTP.get<AssignmentTestsResponse>(
            `/assignments/${assignmentId}/studentTests/`,
        );
    },
    ...
    getSubmissionFiles: async (assignmentId: number) => {
        return await CodePostHTTP.get<AssignmentSubmissionedFilesResponse>(
            `/assignments/${assignmentId}/studentUpload/`,
        );
    },
    Submit: async (
        assignmentId: number,
        ...
        if (sendConfirmationEmail) {
            formData.append(
                ...
                sendConfirmationEmail.toString(),
            );
        }

        return await CodePostHTTP.post<SubmissionStatus>(
            `/assignments/${assignmentId}/studentUpload/`,
            formData,
        );
    },
    addFileToSubmission: async (
        assignmentId: number,
        files: File[],
        sendConfirmationEmail: boolean = false,
    ) => {
        ...
        return await CodePostHTTP.patch<SubmissionStatus>(
            `/assignments/${assignmentId}/studentUpload/`,
            formData,
        );
    },
    getSubmissionTests: async (
        assignmentId: number,
        options?: QueryListParams,
    ) => {
        const params = getQueryParams(options);
        // This might be AssignmentSubmissionTestsResponse
        return await CodePostHTTP.get<SubmissionWithTests>(
            `/assignments/${assignmentId}/submissionTests/`,
            params,
        );
    },
    clone: async (assignmentId: number, courseId: number) => {
        return await CodePostHTTP.post<"Success!">(
            `/assignments/${assignmentId}/clone/`,
            { course: courseId },
        );
    },
};

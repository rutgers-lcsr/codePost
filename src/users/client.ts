...
export const Users = {
    ...
    emailUser: async (email: string, options: EmailRequest) =>
        await CodePostHTTP.post<EmailSuccess, EmailRequest>(`/users/${email}/email/`, options, EmailSuccessSchema),
    updateMe: async (options: UserUpdate) => await CodePostHTTP.patch<User, UserUpdate>(`/users/me/`, options, UserSchema),
    me: async () => await CodePostHTTP.get<User>("/users/me/", {}, UserSchema),
    requestAPIToken: async () => {
        const user = await CodePostHTTP.post<User>("/users/requestAPIToken", {}, UserSchema);
